...
def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level1_39_LessEqual.json')
    ...
    for case in cases:
        ...
        if "data" in input1_info:
            ...
            input1 = torch.rand(input1_info["shape"], dtype=DTYPE_MAP[input1_info["dtype"]]) * (input1_info["range"][1] - input1_info["range"][0]) + input1_info["range"][0]
        if "data" in input2_info:
            input2 = torch.tensor(input2_info["data"], dtype=DTYPE_MAP[input2_info["dtype"]]).reshape(input2_info["shape"])
        else:
            input2 = torch.rand(input2_info["shape"], dtype=DTYPE_MAP[input2_info["dtype"]]) * (input2_info["range"][1] - input2_info["range"][0]) + input2_info["range"][0]
