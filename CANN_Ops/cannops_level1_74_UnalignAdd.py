...
def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level1_74_UnalignAdd.json')
    ...
    for case in cases:
        ...
        if "data" in input1_info:
            ...
            input1 = torch.randn(input1_info["shape"], dtype=DTYPE_MAP[input1_info["dtype"]]) * input1_info["std"] + input1_info["mean"]
        if "data" in input2_info:
            input2 = torch.tensor(input2_info["data"], dtype=DTYPE_MAP[input2_info["dtype"]]).reshape(input2_info["shape"])
        else:
            input2 = torch.randn(input2_info["shape"], dtype=DTYPE_MAP[input2_info["dtype"]]) * input2_info["std"] + input2_info["mean"]
