import torch
import torch.nn as nn
import json
import os

DTYPE_MAP = {
    "float16": torch.float16,
    "float32": torch.float32,
    "float64": torch.float64,
    "bfloat16": torch.bfloat16,
    "int8": torch.int8,
    "int16": torch.int16,
    "int32": torch.int32,
    "int64": torch.int64,
    "uint8": torch.uint8,
    "bool": torch.bool,
    "complex64": torch.complex64,
}

from typing import List, Optional
import torch
import torch.nn as nn
import math

class Model(nn.Module):

    def __init__(self):
        super(Model, self).__init__()

    def forward(self, x: torch.Tensor, grad: torch.Tensor, indices: torch.Tensor, ksize: List[int], strides: List[int], pads: List[int], dilation: List[int], ceil_mode: bool) -> torch.Tensor:
        m = torch.nn.MaxPool3d(kernel_size=ksize, stride=strides, padding=pads, dilation=dilation, return_indices=True, ceil_mode=ceil_mode)
        out, indices = m(x.to(torch.float32))
        gradOut_float = grad.to(torch.float32)
        x_float = x.to(torch.float32)
        grad_in = torch.ops.aten.max_pool3d_with_indices_backward(gradOut_float, x_float, ksize, strides, pads, dilation, ceil_mode, indices.to(torch.int64))
        return grad_in.to(x.dtype)

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_126_MaxPool3DGradWithArgmax.json')
    with open(json_path, "r") as f:
        cases = [json.loads(line) for line in f if line.strip()]

    input_groups = []
    for case in cases:
        inputs = case["inputs"]
        x_info = inputs[0]
        grad_info = inputs[1]
        indices_info = inputs[2]
        ksize_info = inputs[3]
        strides_info = inputs[4]
        pads_info = inputs[5]
        dilation_info = inputs[6]
        ceil_mode_info = inputs[7]

        if "data" in x_info:
            x = torch.tensor(x_info["data"], dtype=DTYPE_MAP[x_info["dtype"]]).reshape(x_info["shape"])
        else:
            x = torch.randn(x_info["shape"], dtype=DTYPE_MAP[x_info["dtype"]])
        if "data" in grad_info:
            grad = torch.tensor(grad_info["data"], dtype=DTYPE_MAP[grad_info["dtype"]]).reshape(grad_info["shape"])
        else:
            grad = torch.randn(grad_info["shape"], dtype=DTYPE_MAP[grad_info["dtype"]])
        if "data" in indices_info:
            indices = torch.tensor(indices_info["data"], dtype=DTYPE_MAP[indices_info["dtype"]]).reshape(indices_info["shape"])
        else:
            indices = torch.randint(indices_info["range"][0], indices_info["range"][1] + 1, tuple(indices_info["shape"]), dtype=DTYPE_MAP[indices_info["dtype"]])
        ksize = ksize_info["value"]
        strides = strides_info["value"]
        pads = pads_info["value"]
        dilation = dilation_info["value"]
        ceil_mode = ceil_mode_info["value"]

        input_groups.append([x, grad, indices, ksize, strides, pads, dilation, ceil_mode])
    return input_groups


def get_init_inputs():
    return []
