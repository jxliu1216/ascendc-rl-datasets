...
class Model(nn.Module):
    ...
    def forward(self, a: torch.Tensor, b: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            两个输入张量的和
        """
        output = torch.matmul(a.to(torch.float32), b.to(torch.float32)) + bias.to(torch.float32)
        return output

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level3_15_MatmulBias.json')
    ...
    for case in cases:
        ...
        if "data" in bias_info:
            ...
            bias = torch.rand(bias_info["shape"], dtype=DTYPE_MAP[bias_info["dtype"]]) * (bias_info["range"][1] - bias_info["range"][0]) + bias_info["range"][0]

        input_groups.append([a, b, bias])
    return input_groups
