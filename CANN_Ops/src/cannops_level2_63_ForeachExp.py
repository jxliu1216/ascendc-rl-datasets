...
class Model(nn.Module):
    """
    Simple model that performs exp operation.
    """

    def __init__(self):
        """
        Initialize the model for exp operation.
        No parameters needed for basic exp operation.
        """
        super(Model, self).__init__()

    def forward(self, inputs: List[torch.Tensor]) -> List[torch.Tensor]:
        """
        Computes the exp of input elements.

        Args:
            inputs: Input tensor list of any shape.

        Returns:
            Output tensor of same shape as input with exp applied elementwise.
        """
        return [torch.exp(x) for x in inputs]

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_63_ForeachExp.json')
