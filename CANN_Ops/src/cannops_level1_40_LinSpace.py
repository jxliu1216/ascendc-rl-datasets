...
class Model(nn.Module):
    ...
    def forward(self, start: torch.Tensor, end: torch.Tensor, steps: int) -> torch.Tensor:
        return torch.linspace(float(start[0].item()), float(end[0].item()), steps, dtype=start.dtype)

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level1_40_LinSpace.json')
    ...
    for case in cases:
        ...
        if "data" in start_info:
            ...
            if _dt in (torch.int8, torch.int16, torch.int32, torch.int64, torch.uint8):
                start = torch.full(start_info["shape"], start_info["fill"], dtype=_dt)
            elif _dt == torch.bool:
        ...
        steps = steps_info["value"]

        input_groups.append([start, end, steps])
    return input_groups
