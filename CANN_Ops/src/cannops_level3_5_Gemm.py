...
class Model(nn.Module):
    ...
    def forward(self, a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, alpha: torch.float32, beta: torch.float32) -> torch.Tensor:
        """
        实现add算子功能。
        ...
        Returns:
            两个输入张量的和
        """
        output = torch.matmul(alpha * a, b) + beta * c
        return output

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level3_5_Gemm.json')
    ...
    for case in cases:
        ...
        input_groups.append([a, b, c, alpha, beta])
    return input_groups
