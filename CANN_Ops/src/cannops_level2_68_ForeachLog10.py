...
class Model(nn.Module):
    """
    Simple model that performs log10 operation.
    """

    def __init__(self):
        """
        Initialize the model for log10 operation.
        No parameters needed for basic log10 operation.
        """
        super(Model, self).__init__()

    def forward(self, inputs: List[torch.Tensor]) -> List[torch.Tensor]:
        """
        Computes the log10 of input elements.

        Args:
            inputs: Input tensor list of any shape.

        Returns:
            Output tensor of same shape as input with log10 applied elementwise.
        """
        return [torch.log10(x) for x in inputs]

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_68_ForeachLog10.json')
