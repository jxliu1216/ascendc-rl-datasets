...
class Model(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor], scalars: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        """
        res = torch._foreach_maximum(x, scalars)
        return [r.to(x[0].dtype) for r in res]

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_73_ForeachMaximumScalarList.json')
