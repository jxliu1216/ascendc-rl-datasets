...
class Model(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return torch.square(torch.nn.functional.relu(input.to(torch.float32))).to(input.dtype)

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_142_SquaredRelu.json')
