...
class Model(nn.Module):
    ...
    def forward(self, grad, x, grid, interpolation_mode='bilinear', padding_mode='zeros', align_corners=False):
        ...
        output = F.grid_sample(x_float, grid_float, mode=interpolation_mode, padding_mode=padding_mode, align_corners=align_corners)
...
def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_108_GridSampler3DGrad.json')
