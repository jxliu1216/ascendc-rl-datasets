...
class Model(nn.Module):
    ...
    def forward(self, grad_output: torch.Tensor, output_size: List[int], input_size: List[int], scales_h: float=0.0, scales_w: float=0.0) -> torch.Tensor:
        ...
        grad_out_flat = grad_output.reshape(N, C, oH * oW).to(compute_dtype)
        src_expanded = src_flat.unsqueeze(0).unsqueeze(0).expand(N, C, -1)
        grad_input_flat = torch.zeros(N, C, iH * iW, device=grad_output.device, dtype=compute_dtype)
...
def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_162_UpsampleNearest2dGrad.json')
