...
class Model(nn.Module):
    ...
    def forward(self, input: torch.Tensor, values: torch.Tensor) -> torch.Tensor:
        return torch.heaviside(input, values)

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_114_Heaviside.json')
    ...
    for case in cases:
        ...
        if "data" in values_info:
            ...
            values = torch.randn(values_info["shape"], dtype=DTYPE_MAP[values_info["dtype"]])

        input_groups.append([input, values])
    return input_groups
