...
class Model(nn.Module):
    """
    Simple model that performs tan operation.
    """

    def __init__(self):
        """
        Initialize the model for tan operation.
        No parameters needed for basic tan operation.
        """
        super(Model, self).__init__()

    def forward(self, inputs: List[torch.Tensor]) -> List[torch.Tensor]:
        """
        Computes the tan of input elements.

        Args:
            inputs: Input tensor list of any shape.

        Returns:
            Output tensor of same shape as input with tan applied elementwise.
        """
        return [torch.tan(x) for x in inputs]

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_97_ForeachTan.json')
