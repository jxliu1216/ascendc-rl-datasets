...
class Model(nn.Module):
    ...
    def forward(self, x: torch.Tensor, output_size: List[int], scale_d: float=0.0, scale_h: float=0.0, scale_w: float=0.0) -> torch.Tensor:
        ...
        d_src = (d_idx * s_d).floor().long().clamp(0, iD - 1)
        h_src = (h_idx * s_h).floor().long().clamp(0, iH - 1)
        w_src = (w_idx * s_w).floor().long().clamp(0, iW - 1)
        return x[:, :, d_src.unsqueeze(1).unsqueeze(1), h_src.unsqueeze(1), w_src]

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level2_163_UpsampleNearest3d.json')
