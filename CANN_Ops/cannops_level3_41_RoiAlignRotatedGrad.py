...
class Model(nn.Module):
    """CPU：对 RoiAlignRotated 前向参考做 autograd，得到对特征 x 的梯度。"""

    def __init__(self):
        super().__init__()

    def forward(self, grad_out: torch.Tensor, rois: torch.Tensor, pooled_h: int, pooled_w: int, spatial_scale: float, sampling_ratio: int, aligned: bool, clockwise: bool, x_feat: torch.Tensor) -> torch.Tensor:
        x = x_feat.detach().float().clone().requires_grad_(True)
        out = _roi_align_rotated_reference(x, rois.float(), int(pooled_h), int(pooled_w), float(spatial_scale), int(sampling_ratio), bool(aligned), bool(clockwise))
        out.backward(grad_out.float())
        return x.grad.to(dtype=x_feat.dtype)

def get_input_groups():
    json_path = os.path.join(os.path.dirname(__file__), 'cannops_level3_41_RoiAlignRotatedGrad.json')
    ...
    for case in cases:
        ...
        if "data" in x_feat_info:
            x_feat = torch.tensor(x_feat_info["data"], dtype=DTYPE_MAP[x_feat_info["dtype"]]).reshape(x_feat_info["shape"])
        else:
            x_feat = torch.randn(x_feat_info["shape"], dtype=DTYPE_MAP[x_feat_info["dtype"]])

        input_groups.append([grad_out, rois, pooled_h, pooled_w, spatial_scale, sampling_ratio, aligned, clockwise, x_feat])
    return input_groups
